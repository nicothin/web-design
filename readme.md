...
**Создавайте компоненты, а не страницы**. Некоторые проекты будут нуждаться в расширении. Гораздо разумнее собирать новые страницы их компонетов, чем создавать дубликаты уже имеющихся (или похожих) элементов.
...
**В макетах допустим только режим наложения normal**. Критично, обязательно. И так будет, пока не [появится кроссбраузерная поддержка](http://caniuse.com/#search=blend) режимов наложения. Это не касается состоящих из множества слоёв иллюстраций, которые планируется соединять в одно изображение.
...
**Идеал модульной сетки — 12 колонок**. Ибо 12 делится на 2, 3, 4 и 6. Чтобы придумывать свои (5-и, 8-колоночные) сетки нужно иметь действительно серьёзные причины, ибо 8-колоночная сетка не позволит выстроить блоки по 3 в ряд.
...
**Символ «рубль» есть во многих шрифтах**. При желании использовать символ рубря в указании цен, не стоит использовать комбинацию «Р-», в которой дефис сильно сдвинут (отрицательным кернингом). Во многих шрифтах есть символ рубля, а если подходящий его вид найти не удалось, нарисуйте векторную картинку и отдайте её технологу в формате svg.
...
**Уточняйте в ТЗ как ведёт себя дизайн на разных брейкпоинтах**. По умолчанию, всё, что уже чем ≈1000 пикс. будет «резиновым».
